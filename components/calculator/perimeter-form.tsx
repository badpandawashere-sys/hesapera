             ...
             <div className="space-y-3 mt-4">
                {(inputs.sekil!=='Daire') && (
                  <div className="space-y-1">
                    ...
                    <input type="text" inputMode="decimal" value={inputs.kenarA} onChange={e=>update('kenarA', e.target.value)} className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl outline-none text-white focus:border-indigo-500" />
                  </div>
                )}
                {(['Dikdörtgen', 'Üçgen', 'Paralelkenar', 'Yamuk'].includes(inputs.sekil)) && (
                  <div className="space-y-1">
                    ...
                    <input type="text" inputMode="decimal" value={inputs.kenarB} onChange={e=>update('kenarB', e.target.value)} className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl outline-none text-white focus:border-indigo-500" />
                  </div>
                )}
                {(['Üçgen', 'Yamuk'].includes(inputs.sekil)) && (
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-indigo-400">3. Kenar</label>
                    <input type="text" inputMode="decimal" value={inputs.kenarC} onChange={e=>update('kenarC', e.target.value)} className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl outline-none text-white focus:border-indigo-500" />
                  </div>
                )}
                {inputs.sekil==='Yamuk' && (
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-indigo-400">4. Kenar</label>
                    <input type="text" inputMode="decimal" value={inputs.kenarD} onChange={e=>update('kenarD', e.target.value)} className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl outline-none text-white focus:border-indigo-500" />
                  </div>
                )}
                {inputs.sekil==='Daire' && (
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-indigo-400">Yarıçap (r)</label>
                    <input type="text" inputMode="decimal" value={inputs.yaricap} onChange={e=>update('yaricap', e.target.value)} className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl outline-none text-white focus:border-indigo-500" />
                  </div>
                )}
             </div>
          </div>
